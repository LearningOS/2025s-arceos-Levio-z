...
fn sys_mmap(
    ...
    let vaddr = align_up(addr as usize, PAGE_SIZE_4K);
    let end = align_up(vaddr +length, PAGE_SIZE_4K);
    let size =end - vaddr;
    ax_println!("{:#x} - {:#x}", vaddr, end);
    ...
    uspace
        .map_alloc(
            vaddr,
            size,
            MmapProt::from_bits_truncate(prot as i32).into(),
            true,
        ).unwrap();

    ax_println!("{:#x} - {:#x}", vaddr, vaddr + size);

    let mut buf = vec![0u8; size];

    api::get_file_like(fd).unwrap().read(&mut buf);
    uspace.write(vaddr, &buf).unwrap();
